...
public class HideDAO {
    ...
    public static List<Discussion> findHidedDiscussionListByIdUser(int id_user) {
        try {
            ResultSet discussion_data = executeSQLQuerry(
                "SELECT * FROM Discussion d NATURAL JOIN Hide h WHERE h.id_user = " + id_user
            );
    ...
    public static void addHideDiscussionByIdDiscussion(int id_discussion, int id_user) throws SQLException {
        try {
            excuteInsertSQL(
                "INSERT OR ROLLBACK INTO Hide VALUES (" + id_user + "," + id_discussion + ")"
            );
        } catch (SQLException e) {
            System.out.println("[HideDAO] Impossible de créer le lien dans Hide : " + e.getMessage());
            throw e;
        }
    }
